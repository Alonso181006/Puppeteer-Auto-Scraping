...
(async () => {
    ...
    for (let i = 0; i < companies.length; i++) {
        ...
        // Domain Check
        const searchVariantsRV = [legalName, companyName, companyName.replace(/\s+/g, ''), legalName.replace(/\s+/g, '')];
        urlRV = await findRVUrl(page, typeBoxRV, linkSelectorRV, domain, searchVariantsRV);

        ({customsBrokersStr, totalShipments, totalClearances, tradeDataStr, portTypeStr} = await RVScraping(urlRV, page, domain, customsBrokersStr, totalShipments, totalClearances, tradeDataStr, portTypeStr));

        // Import Yeti
        const searchVariants = [legalName];
        if (i % 5 === 0) {
            await importYetiLogIn(page);
        }
        ...
        if ( i % 2 === 0)  {
            ...
            const csvLines = resultsToCSV(results);
...
async function RevVesLogIn (page){
    // 2. Log in - Rev Vessel
...
async function findRVUrl(page, typeBoxRV, linkSelectorRV, domain, searchVariants) {
        await clearAndType(page, typeBoxRV, domain);
        let urlRV = await getImporterDetailURL(page, linkSelectorRV, domain);
        ...
        if (!urlRV) {
            ...
            for (const term of searchVariants) {
                if (urlRV) break; // Already found a match
                await clearAndType(page, typeBoxRV, term);
                urlRV = await getImporterDetailURL(page, linkSelector);
            }
        }
        return urlRV;
}
...
async function RVScraping(urlRV, page, domain, customsBrokersStr, totalShipments, totalClearances, tradeDataStr, portTypeStr) {
    if(urlRV) {
        await page.goto(urlRV, { waitUntil: 'networkidle2' });
    ...
    return {customsBrokersStr, totalShipments, totalClearances, tradeDataStr, portTypeStr}
}
